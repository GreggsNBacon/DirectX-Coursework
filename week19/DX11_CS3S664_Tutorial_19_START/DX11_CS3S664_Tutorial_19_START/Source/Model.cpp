...
void Model::load(ID3D11Device *device, Effect *_effect, const std::wstring& filename ) {
			...
			for (uint32_t i = 0; i < numMeshes; ++i) 
			{
				...
				for ( j = 0; j < mesh->mNumFaces; ++j)		
				{
					...
					for (int k = 0; k < 3; ++k)
					{
						...
						aiVector3D uv = mesh->HasTextureCoords(0) ? uv = mesh->mTextureCoords[0][face.mIndices[k]] : aiVector3D(1.0f, 1.0f, 1.0f);
