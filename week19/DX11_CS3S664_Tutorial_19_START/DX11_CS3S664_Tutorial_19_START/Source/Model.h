...
public:
	...
	Model(ID3D11Device *device, const std::wstring& filename, Effect *_effect, Material *_materials[] = nullptr, int _numMaterials = 0, ID3D11ShaderResourceView **textures = nullptr, int numTextures = 0) : BaseModel(device, _effect, _materials, _numMaterials, textures, numTextures){ load(device, _effect, filename); }
	~Model();
	void load(ID3D11Device *device, Effect *_effect, const std::wstring& filename);
