...
HRESULT Scene::initialiseSceneResources() {
	...
	Texture* sandText = new Texture(device, L"Resources\\Textures\\beach_sand.png");
	...
	ID3D11ShaderResourceView *grassTextureArray[] = { grassTexture->getShaderResourceView(), grassAlpha->getShaderResourceView(), sandText->getShaderResourceView()};
	...
	castle->setWorldMatrix(castle->getWorldMatrix()* XMMatrixScaling(10, 10, 10)* XMMatrixTranslation(-40, 9, 0));
	castle->update(context);

	terrain = new Terrain(device, context, 500, 500, heightMap->getTexture(), normalMap->getTexture(), grassEffect, NULL, 0, grassTextureArray, 3);
	...
	//cBufferLightCPU->lightDiffuse = XMFLOAT4(1.886, 1.345, 1.133, 1.0);
	//cBufferLightCPU->lightSpecular = XMFLOAT4(0.5, 0.5, 0.5, 0.5);
	//cBufferLightCPU->lightAttenuation = XMFLOAT4(1, 0.0, 0.02, 60);//const = 0.0; linear = 0.0; quad = 0.05; cutOff = 200.0;
	cBufferLightCPU->lightVec = XMFLOAT4(-40, 15, 20, 1.0);
	cBufferLightCPU->lightAmbient = XMFLOAT4(0.0, 0.0, 0.0, 1.0);
	cBufferLightCPU->lightDiffuse = XMFLOAT4(1.886, 1.345, 1.133, 1.0);
	cBufferLightCPU->lightSpecular = XMFLOAT4(0.7, 0.7, 0.7, 0.7);
	cBufferLightCPU->lightAttenuation = XMFLOAT4(1, 0.0, 0.02, 1);
