...
Texture2D diffuse1Texture : register(t2);
...
FragmentOutputPacket main(FragmentInputPacket v) {
	...
	float4 baseColour = v.matDiffuse;
		...
		colour1 = diffuse1Texture.Sample(anisotropicSampler, v.texCoord);
	...
	colour += att* max(dot(lightDir, N), 0.0f) * baseColour.xyz * lightDiffuse;
